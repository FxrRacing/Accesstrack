...
import { importUsers, ValidationError } from "./actions";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function ImportUsers({ authId }: { authId: string }) {
  ...
  const downloadSample = async () => {
    const res = await fetch("/api/users/sample");
    if (!res.ok) {
    ...
    a.href = url;
    a.download = "users_sample_data.csv";
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const result = await importUsers(text, authId);
  
      if (result.success) {
        toast.success(result.message);
      } else {
        // Show validation errors in a more detailed way
        const errorMessage = result.validationErrors.length > 0
          ? `Validation failed for ${result.validationErrors.length} rows:\n` +
            result.validationErrors.map((err: ValidationError) => 
    ...
    <>
      <Dialog>
        <DialogTrigger asChild>
          <Button className="bg-black text-white hover:bg-black/80 rounded-full px-6 cursor-pointer">
            ...
            <div>
              ...
              >
                ...
                <p className="text-gray-600 text-center mb-2">
                  ...
                  <span className="text-purple-500 font-semibold">browse</span>
                </p>
              ...
              {error && (
                <Alert
                  variant="destructive"
                  className="mt-4 rounded-xl border border-red-200 bg-red-50"
                >
                              ...
                              <div>
                                ...
                                {preview.length > 0 && (
                                  <p className="text-sm text-gray-600 mt-1">
                                    A total of {preview.length - 1} rows will be
                                    added to your data
                                  </p>
                                )}
                                <p className="text-sm text-gray-500 mt-1">
                                  Here is a preview of the data that will be
                                  added (showing the first 8 rows).
                                </p>
                              </div>
                  ...
                  <TabsContent value="file">
                    ...
                    <div className="mt-4 bg-gray-50 rounded-xl p-4 border border-gray-100">
                      ...
                      <p className="text-xs text-gray-500 ml-6">
                        Your CSV file is valid and ready to be imported
                      </p>
                    </div>
                  </TabsContent>
                </Tabs>
